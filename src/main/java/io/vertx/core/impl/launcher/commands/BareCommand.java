...
public class BareCommand extends ClasspathHandler {
  ...
  protected Vertx startVertx() {
    ...
    configureFromSystemProperties.set(true);
    try {
  ...
  private static final ThreadLocal<Boolean> configureFromSystemProperties = new ThreadLocal<>();

  public static void configureFromSystemProperties(Object options, String prefix) {
    if (configureFromSystemProperties.get() != Boolean.TRUE) {
      return;
    }
